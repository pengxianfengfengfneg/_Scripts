local PetAttrView = Class(game.BaseView)

local type_image = { "zs_wai", "zs_nei", "zs_ping" }
local gen_type = { "zs_01", "zs_02", "zs_03" }

function PetAttrView:_init(ctrl)
    self._package_name = "ui_pet"
    self._com_name = "pet_attr_view"
...
function PetAttrView:OpenViewCallBack(pet_info, open_index)
    self:InitBg()
    self:InitTemplate()
    self:InitBtns()
    self:InitPetList()
...
function PetAttrView:RegisterAllEvents()
    local events = {
        {
...
function PetAttrView:SetPetInfo(info)
...
function PetAttrView:InitPetList()
...
function PetAttrView:SetPetList()
...
function PetAttrView:InitBg()
    self:GetFullBgTemplate("common_bg"):SetTitleName(config.words[1654])
end

function PetAttrView:InitTemplate()
    self.attr_template = self:GetTemplateByObj("game/pet/pet_attr_template", self._layout_objs.page:GetChildAt(0))
    self.skill_template = self:GetTemplateByObj("game/pet/pet_skill_template", self._layout_objs.page:GetChildAt(1))

    self._layout_objs.page:SetHorizontalBarTop(true, 21)
end

function PetAttrView:CalcFight(pet_info)
    local fight = 0
