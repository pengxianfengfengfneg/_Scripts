local TaskCom = Class(game.UITemplate)
...
function TaskCom:_init()
	TaskCom.instance = self
end
    
function TaskCom:_delete()
	TaskCom.instance = nil
end

function TaskCom:OpenViewCallBack()
	self:Init()
	
	self:RegisterAllEvents()
end

function TaskCom:CloseViewCallBack()
...
function TaskCom:RegisterAllEvents()
	local events = {
...
function TaskCom:OnUpdateDailyTask(daily_task_type, ...)
...
function TaskCom:UpdateTaskList()
	local new_task_list = {}
...
function TaskCom:GetTaskData(idx)
	return self.task_data[idx]
end

function TaskCom:OnUpdateTaskInfo(data)	
    --self:UpdateTaskList()

    self.next_task_update_time = global.Time.now_time + 0.2
end

function TaskCom:OnAcceptTask()
	--self:UpdateTaskList()
	self.next_task_update_time = 0
end

function TaskCom:OnGetTaskReward(task_id, is_main_task)
	--self:UpdateTaskList()
	self.next_task_update_time = global.Time.now_time + 0.5
end

function TaskCom:OnDoneTaskTalk(task_id)
	--self:UpdateTaskList()
	self.next_task_update_time = global.Time.now_time + 0.5
end

function TaskCom:OnHangTask(task_id, is_hang)
	if is_hang then
...
function TaskCom:ShowTask(task_id)
...
function TaskCom:GetShowingTask()
	return self.showing_task_id
end

function TaskCom:RefreshShowingTask()
	local show_task_id = self:CalcShowingTask()
	self:ShowTask(show_task_id)
end

function TaskCom:CalcShowingTask()
...
function TaskCom:Update(now_time, elapse_time)
