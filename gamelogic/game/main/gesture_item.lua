...
function GestureItem:OpenViewCallBack()
	...
	self.joystick_info = {
		com = self._layout_objs["joystick"],
		node = self._layout_objs["joystick/joystick_node"],
		center_pos = cc.vec2(0, 0),
		radius = 65,
		new_dir = cc.vec2(0, 0),
		pre_dir = cc.vec2(0, 0),
		next_move_time = 0,
...
function GestureItem:OnJoystickTouchMove(x, y, double, touch_id)
	...
	if self.touch_info.touch_mode == 1 then
		...
		if len > self.joystick_info.radius then
			...
			self.joystick_info.node:SetPosition(dx + self.joystick_info.radius, dy + self.joystick_info.radius)
		end
