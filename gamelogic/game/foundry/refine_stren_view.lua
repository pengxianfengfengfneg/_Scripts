local RefineStrenView = Class(game.BaseView)

function RefineStrenView:_init(ctrl)
    self._package_name = "ui_equip_refine"
    self._com_name = "refine_stren_view"
...
function RefineStrenView:_delete()
end

function RefineStrenView:OpenViewCallBack()
...
function RefineStrenView:CloseViewCallBack()
    self:DeleteItems()
...
function RefineStrenView:InitItems()
...
function RefineStrenView:DeleteItems()
...
function RefineStrenView:InitTopStrenEquips()
    ...
    for pos, v in pairs(config.equip_paris) do
        ...
        if equip_info.paris > 0 then
            ...
            local next_cfg = config.equip_paris[pos][equip_info.paris+1]
            if next_cfg then
                count = count + 1
...
function RefineStrenView:OnSelectEquip(index)
    self.select_index = index
    self:ShowInStren()
end


function RefineStrenView:ShowInStren()
    ...
    if next_cfg then
        ...
        self.target_item:AddClickEvent(function()
            game.BagCtrl.instance:OpenWearEquipInfoCompareView(equip_info, true, {paris = equip_info.paris + 1})
        end)
...
function RefineStrenView:OnDefaultClickTop()
...
function RefineStrenView:OnClickTab()
...
function RefineStrenView:PlayEffect()
    self._layout_objs.effect:SetVisible(true)
    self:CreateUIEffect(self._layout_objs.effect, "effect/ui/zb_clqh.ab")
end


return RefineStrenView
